...
class UniBEVTransformer(BaseModule):
    ...
    def init_layers(self):
        ...
        if self.feature_norm == "ChannelNormWeights":
            ...
            self.channel_weights_proj = nn.Sequential(
                nn.Linear(self.bev_h * self.bev_w * 2, 2), nn.LeakyReLU(inplace=True)
            )
    ...
    def init_weights(self):
        ...
        for m in self.modules():
            if (
                isinstance(m, MSDeformableAttention3DPts)
                or isinstance(m, MSDeformableAttention3DImg)
                or isinstance(m, MultiScaleDeformableAttention)
        ...
        if self.feature_norm in (
            "MLP_ChannelNormWeights",
        ...
        if self.use_modal_embeds == "MLP":
            xavier_init(self.modal_embbeding_mlp, distribution="uniform", bias=0.0)
        elif self.use_modal_embeds == "Fixed":
    ...
    def _pre_process_img_feats(self, mlvl_img_feats, bev_queries):
        ...
        img_level_start_index = torch.cat(
            (img_spatial_shapes.new_zeros((1,)), img_spatial_shapes.prod(1).cumsum(0)[:-1])
        )
    ...
    def _pre_process_pts_feats(self, mlvl_pts_feats, bev_queries):
        ...
        pts_level_start_index = torch.cat(
            (pts_spatial_shapes.new_zeros((1,)), pts_spatial_shapes.prod(1).cumsum(0)[:-1])
        )
    ...
    def multi_modal_fusion(self, img_bev_embed, pts_bev_embed):
        ...
        if self.fusion_method == "linear":
            ...
            fused_bev_embed = img_bev_embed * self.c_flag / (
                self.c_flag + self.l_flag
            ) + pts_bev_embed * self.l_flag / (self.c_flag + self.l_flag)
        elif self.fusion_method == "cat":
            if self.feature_norm == "ModalityProjection":
                assert (
                    img_bev_embed.shape[-1] == self.embed_dims * 2 and pts_bev_embed.shape[-1] == self.embed_dims * 2
                )
    ...
    def forward(
        ...
        **kwargs
        ...
        if img_mlvl_feats is not None:
            img_feat_flatten, img_spatial_shapes, img_level_start_index = self._pre_process_img_feats(
                img_mlvl_feats, bev_queries_img
            )
            img_bev_embed = self.img_bev_encoder(
                ...
                **kwargs
            )  # encoder.batch_first = True: (bs, bev_h*bev_w, embed_dims)
        else:
            img_bev_embed = None

        if pts_mlvl_feats is not None:
            pts_feat_flatten, pts_spatial_shapes, pts_level_start_index = self._pre_process_pts_feats(
                pts_mlvl_feats, bev_queries_pts
            )
