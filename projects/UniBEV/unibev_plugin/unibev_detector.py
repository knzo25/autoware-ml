...
class UniBEV(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        if radar_voxel_encoder:
            self.radar_voxel_encoder = MODELS.build_voxel_encoder(radar_voxel_encoder)
        if radar_middle_encoder:
            self.radar_middle_encoder = MODELS.build_middle_encoder(radar_middle_encoder)
    ...
    def forward_train(
        ...
        **kwargs,
        ...
        img_feats, lidar_feats, radar_feats = self.extract_feat(
            img=img, points=points, radar_points=radar, img_metas=img_metas
        )
        ...
        if self.use_lidar == True and self.use_radar == False:
            ...
            raise ValueError(
                "Unsupported Modality Mode: Cam: {}, Lidar:{}, Radar:{}".format(
                    self.use_camera, self.use_lidar, self.use_radar
                )
            )
        else:
            pts_feats = None
